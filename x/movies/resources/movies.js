...
export function focusItems() {
    if (window._moviesTabFocusItems) {
        window._moviesTabFocusItems();
    }
}
...
function organizeIntoGridItems(movies) {
    ...
    // Re-sort so collection cards use their collection name as the sort key,
    // not the sort_title of their first member (which may differ).
    // This matches what collapseCollections() does for list view.
    if (sortOrder === 'title') {
        const sortKey = s => {
            const k = s.replace(/^the\s+/i, '');
            return k;
...
function setupEventListeners() {
    ...
    document.querySelectorAll('.movies-sort-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            // Only handle clicks on buttons with data-sort attribute
    ...
    if (el.collapseCheck) {
        // Initialize groupCollections from checkbox state
