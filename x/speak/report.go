...
func (svc *Service) maybeSendSpeakReport(messenger core.MessengerApi, now time.Time, force bool) error {
	...
	if template == "" {
		logger.Info("speak: report_queue_file not configured; skipping report")
		return nil
	}
	...
	if err != nil {
		logger.Warn("speak: no queue file for previous day", "path", queuePath, "error", err)
		return nil
	}
	...
	if err := scanner.Err(); err != nil {
		logger.Warn("speak: error scanning queue file", "path", queuePath, "error", err)
	}

	md := fmt.Sprintf("# speak report for %s\n", reportDay.Format("2006-01-02"))
	...
	logger.Info("speak: sent nightly report", "date", reportDay.Format("2006-01-02"))
	return nil
}
