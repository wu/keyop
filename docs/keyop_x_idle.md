...
Package idle monitors macOS user idle/active state and provides utilities for recording activity, persisting per\-day summaries, and generating nightly idle reports.

Overview The idle package uses macOS APIs to determine whether the local user is currently "active" or "idle" \(based on a configurable threshold\). On each periodic Check\(\) the service emits an "idle\_status" message and associated metrics; these messages are used to derive per\-host active periods and to generate daily reports.

Configuration The service can be configured via the ServiceConfig.Config map. Supported configuration keys \(all optional unless noted\):

- threshold: string \(duration\) The duration after which the user is considered idle, e.g. "5m". If omitted or unparsable the default is 5m.

- idle\_metric\_name: string The metric name used when emitting idle duration metrics. If omitted a default of \<service\>.idle\_duration is used.

- active\_metric\_name: string The metric name used when emitting active duration metrics. If omitted a default of \<service\>.active\_duration is used.

- report\_queue\_file: string Optional template path to a queue file containing serialized messages for a given day. The template may include the token "yyyymmdd" which will be replaced with a date in 20060102 format \(for example: "\~/logs/keyop/queue.yyyymmdd.log"\). If queue\_file is not configured nightly reports are skipped.

Queue file format \(events\) For nightly reporting the package can read a queue file that contains one JSON message per line. Each line may be either a legacy core.Message JSON object or a core.Envelope \(the code will attempt to unmarshal an Envelope first and fall back to a Message\). Lines that cannot be parsed are ignored. Only messages with Event == "status\_update" and timestamps that fall within the target day are considered when constructing reports.
...
Report generation Nightly reports are generated by maybeSendIdleReport and are intended to run once per day \(the service attempts generation between 00:00 and 01:00 local time for the previous calendar day\). When generating a report the service will:

- determine the queue file path by replacing the token "yyyymmdd" in the configured queue\_file template with the previous day's date
...
- construct per\-host active periods by tracking transitions between status values and aggregating contiguous active spans
- merge active periods across hosts to compute total active time and create coverage intervals to calculate known vs unknown seconds
- compute totals \(active, idle, unknown\) and format a Markdown report that is emitted as a core.Message with Event == "idle\_report"
...
Usage Initialize the service via NewService\(deps, cfg\) and provide the desired config keys described above. Register payloads with a core.PayloadRegistry using RegisterPayloads. The service's Check\(\) method should be invoked on a periodic schedule by the host runtime \(the package expects to run only on macOS/darwin\).
...
<a name="Event"></a>
## type [Event](<https://github.com/wu/keyop/blob/main/x/idle/service.go#L33-L39>)
...
### func \(Event\) [PayloadType](<https://github.com/wu/keyop/blob/main/x/idle/service.go#L42>)
...
maybeSendIdleReport sends a daily report for the previous day between 00:00 and 01:00 local time. If force is true the report will be generated regardless of the current hour \(used for reportOnStartup\).
